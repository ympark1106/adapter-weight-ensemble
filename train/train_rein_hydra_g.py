...
class SWAGManager:
    ...
    def update(self):
        ...
        for key, param in state_dict.items():
            param = param.clone().detach().to(self.device)
            if self.mean is None:
                self.mean = {k: torch.zeros_like(v) for k, v in state_dict.items()}
                self.sq_mean = {k: torch.zeros_like(v) for k, v in state_dict.items()}
            self.mean[key] += (param - self.mean[key]) / (self.n_models + 1)
            self.sq_mean[key] += (param ** 2 - self.sq_mean[key]) / (self.n_models + 1)
        self.n_models += 1

    def sample(self):
        sampled_state_dict = {}
        for key in self.mean.keys():
            variance = self.sq_mean[key] - self.mean[key] ** 2
            sampled_state_dict[key] = self.mean[key] + torch.sqrt(variance + 1e-5) * torch.randn_like(self.mean[key])
...
def train():
    ...
    swag_manager = SWAGManager(model, device)
    ...
    final_weights = swag_manager.sample()
    model.load_state_dict(final_weights)
    torch.save(final_weights, os.path.join(save_path, "swag_final_weights.pth"))
    print("SWAG final model loaded.")    
